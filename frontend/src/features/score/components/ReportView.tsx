...
export function ReportView({ id }: { id: string }) {
  ...
  useEffect(() => {
    // 1. The built-in demo link always shows the sample report.
    if (id === "demo") {
    ...
    // 2. A report we just computed is stashed in the browser — show it instantly
    //    (works even before a database is set up).
    try {
      const cached = sessionStorage.getItem(`report:${id}`);
      if (cached) {
        setReport(JSON.parse(cached) as ScoreReport);
        setStatus("ready");
        return;
      }
    } catch {
      /* sessionStorage unavailable — fall through to the backend */
    }

    // 3. Otherwise ask the backend (this needs the database to be configured).
    api
      .getReport(id)
      .then((r) => {
        setReport(r);
    ...
    return (
      <Section><Container className="flex flex-col items-center gap-4 text-center">
        ...
        <p className="max-w-prose text-neutral">
          This report isn&apos;t saved yet (a database is needed to keep reports across visits).
          Upload your resume again to get a fresh score.
        </p>
    ...
    <>
      <Section className="pb-8 text-center">
        <Container className="flex flex-col items-center gap-4">
          ...
          {report.jobTitle && <p className="text-sm text-neutral">Scored against the job description you pasted</p>}
        </Container>
      </Section>
